...
$aggregate = $kp_service->get_all_konkurs_prowadzacych_data_for_user($user->get_usos_id(), intval($_GET['id']));

if ($aggregate === null) {
    http_response_code(404);
    echo json_encode([
        "error" => "Nie znaleziono takiego konkursu lub został on już rozstrzygnięty"
    ]);
    die();
}

echo json_encode($aggregate->to_array());
?>